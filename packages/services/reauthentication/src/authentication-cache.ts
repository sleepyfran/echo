...
import { Effect, Layer, SubscriptionRef, Option, Stream } from "effect";
 ...
 * refresher service.
 */
export const AuthenticationCacheLive = Layer.effect(
  AuthenticationCache,
  Effect.gen(function* () {
    ...
    yield* authStream.pipe(
      ...
      Stream.runForEach(({ providerId, authInfo }) =>
        SubscriptionRef.update(cache, (cache) =>
    ...
    return AuthenticationCache.of({
      get: (providerId) =>
        SubscriptionRef.get(cache).pipe(
          Effect.map((c) => Option.fromNullishOr(c.get(providerId))),
          Effect.tap((result) =>
